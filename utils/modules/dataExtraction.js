...
const dataExtraction = async (
  ...
  const paramsForInfoExtraction = {
    inputmessage: `
            ...
            - **Company Abbreviations:** The Company abbreviation used for the The full name of the company mentioned in the document(which you extracted earlier) within the document, (both uppercase and lowercase, for example if there is iCW and ICW for iCreditWorks, add both to it (iCW, ICW) etc), make sure it is an abbreviation of the company name(and also not a number), for example MPL cannot be an abbreviation of the company name iCreditWorks(so MPL should not be added), if you realise it is not an abberviation of the company name you extracted then do not add it to the array, if you did not extract any company name(or if you do not see any company name in the document) then return an empty array for abbreviation(if company name(which you extracted) is an empty array then company abbreviation should also be an empty array), add it in this format([name1, name2, name3] and not like ["name1", "name2", "name3"]).
