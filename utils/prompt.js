...
const SystemPrompt = (uploadedFileId, data, fileExt, fileName, selectedTemplate, user_inputs, extractedDataFromDocument) => {
    ...
    const instrunctions = buildPrompt(extractedDataFromDocument, data, user_inputs)
    return `
    Context: You are an expert in document formatting and text replacement. Your task is to update a given document based on specific user instructions while maintaining the original design, style, font, and format.
            ...
            **Task Instructions:**
            ...
            4. **Output Format:**
            - The document is provided in the ${fileExt?.includes('.pdf') ? 'docx' : selectedTemplate?.file?.ext} format. 
            - Generate the updated document in the same ${fileExt?.includes('.pdf') ? 'docx' : selectedTemplate?.file?.ext} format. 
            - Name the new file as ${removeFileExtension(fileName) + '_user_' + data?.user}.
...
const buildPrompt = (extractedDataFromDocument, data, user_inputs) => {
    ...
    const title = extractedDataFromDocument && !extractedDataFromDocument?.title.includes("Not available")
        ? `Replace the document's title as "${extractedDataFromDocument.title}" with "${data?.title}". Ensure the new title matches the original in font size, style, and appears prominently on the first page or as the first line.`
    ...
    const companyNameInstruction = extractedDataFromDocument && !extractedDataFromDocument.companyName.includes("Not available")
    ...
    const companyEmailInstruction = extractedDataFromDocument && !extractedDataFromDocument.companyEmail.includes("Not available")
    ...
    return `
            ...
            - **Additional Inputs:** Integrate the following additional user inputs wherever appropriate: ${user_inputs}.
            `;
};
